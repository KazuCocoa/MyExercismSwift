...
private extension tm {
    ...
    var time: (hour: Int32, mins: Int32, sec: Int32) {
    return (hour: tm_hour, mins: tm_min, sec: tm_sec)
    }
    ...
    mutating func dateByAddingSeconds(secounds: Int) -> tm {
        var d = timegm(&self) + secounds
        return gmtime(&d).memory
    }

    private func addLeadingZero(input: Int32) -> String {
        var addZero = false
        (0...9).forEach {
            if $0 == Int(input) { addZero = true }
        }
    ...
    var description: String {
        let date = [year, month, day, time.hour, time.mins, time.sec].map { addLeadingZero($0) }
...
struct Gigasecond: Equatable, CustomStringConvertible {
    ...
    init?(from: String) {
        if let result = parse(from) {
            var start = result
            startDate = start
            gigasecondDate = start.dateByAddingSeconds(1_000_000_000)
        } else {
            return nil
        }
    }
